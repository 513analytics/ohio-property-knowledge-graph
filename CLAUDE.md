...
│   ├── cinci-housing.ttl        # Main ontology (OWL): ~637 lines, classes for parcels, ownership, assessment, taxation
│   └── cinci-land-use-codes.ttl # Reference vocabulary (SKOS): ~628 lines, land use codes 1xx–8xx
├── docs/                        # Documentation
...
All project ontology entities use prefix `cinci:` → `http://kg.513analytics.com/ont/cinci#`
...
1. Load `ontology/cinci-housing.ttl` and `ontology/cinci-land-use-codes.ttl`
...
All docs reference the ontology via property names like `cinci:landUseCode`, `cinci:taxYear`, etc.

## Namespace Prefixes (for SPARQL)

```sparql
PREFIX cinci: <http://kg.513analytics.com/ont/cinci#>
...
1. Edit `ontology/cinci-housing.ttl` (Turtle format)
2. Maintain namespace prefix consistency (all new entities use `cinci:` prefix)
...
- [ ] Maintain `cinci:` namespace prefix
